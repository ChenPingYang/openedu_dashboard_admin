...
def doGet(request):
    ...
    with connections['ResultDB'].cursor() as cursor:
        ...
        numberOfVideosViewed_Taiwan = '{:,.2f}'.format(int(result[-1].影片觀看人數台灣))
        numberOfVideosViewed_Foreign = '{:,.2f}'.format(int(result[-1].影片觀看人數_非台灣))
        NOVV_moreThanHalf_Taiwan = '{:,.2f}'.format(int(result[-1].影片觀看過半人數_台灣))
        NOVV_moreThanHalf_Foreign = '{:,.2f}'.format(int(result[-1].影片觀看過半人數_非台灣))
        timesOfVideosViewed_Taiwan = '{:,.2f}'.format(int(result[-1].影片觀看人次_台灣))
        timesOfVideosViewed_Foreign = '{:,.2f}'.format(int(result[-1].影片觀看人次_非台灣))
        MobileVehicleViewed = '{:,.2f}'.format(int(result[-1].行動觀看人次))
        ...
        to_render['numberOfVideosViewed_Taiwan'] = numberOfVideosViewed_Taiwan
        to_render['numberOfVideosViewed_Foreign'] = numberOfVideosViewed_Foreign
        to_render['NOVV_moreThanHalf_Taiwan'] = NOVV_moreThanHalf_Taiwan
        to_render['NOVV_moreThanHalf_Foreign'] = NOVV_moreThanHalf_Foreign
        to_render['timesOfVideosViewed_Taiwan'] = timesOfVideosViewed_Taiwan
        to_render['timesOfVideosViewed_Foreign'] = timesOfVideosViewed_Foreign
        to_render['MobileVehicleViewed'] = MobileVehicleViewed
    ...
    with connections['SurveyDB'].cursor() as cursor:
        ...
        for i in range(len(result)):
            ...
            jsonQuestionThree = jsonProblem[2]
            answer3 = str(jsonQuestionThree['answer'])
