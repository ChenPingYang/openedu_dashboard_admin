...
def doGET(request):
    ...
    with connections['ResultDB'].cursor() as cursor:
        ...
        for rs in result:
            ...
            jsonArray_RegisteredPersons.append(dict2json.copy())
        # ----------------------------------------------------
        ...
        for i in range(len(avgRadar_percent)):
            ...
            Data.append('{:,.2f}'.format(maxRadarSite[i] / totalCourceNumber * 100) + '%')
            Data.append('{:,.2f}'.format(avgRadar[i]) + "(" + '{:,.2f}'.format(avgRadar_percent[i]) + '%)')
            Data.append('{:,}'.format(RadarColumn[i]) + '(' + '{:,.2f}'.format(RadarColumn_percent[i]) + '%)')
