...
def removeExtemeValueForAnalysisDiscussion(data):
    absolute_deviation = []
    output = []

    # 取得中位數
    if len(data) % 2 == 1:
        Median = data[int((len(data)+1) / 2)][7]
    else:
        Median = (data[int(len(data) / 2)][7] + data[int(len(data) / 2) + 1][7]) / 2

    # 取得所有數值和中位數間的距離
    for i in range(len(data)):
        absolute_deviation.append(abs(data[i][7] - Median))

    # 進行排列
    absolute_deviation.sort()

    # 取得絕對距離中的中位數(MAD)
    if len(absolute_deviation) % 2 == 1:
        MAD = absolute_deviation[int(len(absolute_deviation) / 2) + 1]
    else:
        MAD = (absolute_deviation[int(len(absolute_deviation) / 2)] +
               absolute_deviation[int(len(absolute_deviation) / 2) + 1]) / 2

    # 如果MAD是零的話，取最小值替代
    if MAD == 0:
        for i in range(len(absolute_deviation)):
            if absolute_deviation[i] != 0:
                MAD = absolute_deviation[i]
                break

    # 算出Z分數
    for i in range(len(data)):
        Z = abs(absolute_deviation[i] - Median) / (MAD / 0.6745)
        if Z < 2.24:
...
def getMedian(data):
    ...
    if (num + len(data)) % 2 == 1:
        Median = data[int((len(data) + num + 1) / 2)][7]
    else:
        Median = (data[int((len(data) + num) / 2)][7] + data[int((len(data) + num) / 2) + 1][7]) / 2
